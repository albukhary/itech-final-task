...
#  image_id      = var.ami_id
  ...
  user_data = base64encode(templatefile("${path.module}/userdata.tpl", { CLUSTER_NAME = aws_eks_cluster.eks-cluster.name, B64_CLUSTER_CA = aws_eks_cluster.eks-cluster.certificate_authority[0].data, API_SERVER_URL = aws_eks_cluster.eks-cluster.endpoint }))
}
